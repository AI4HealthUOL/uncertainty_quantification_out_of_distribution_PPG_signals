...
class ResBlock_MCD_original(nn.Module):
    "Resnet block from `ni` to `nh` with `stride`"
...
class ResBlock_MCD_1(nn.Module): # can  solve the problem of see4 but for dr==0 we have problem
    "Resnet block from `ni` to `nh` with `stride`"
    def __init__(self, expansion, ni, nf, stride=1, kernel_size=3, groups=1, nh1=None, nh2=None, dw=False, g2=1, dropout_prob = 0.05,
                 norm_type=NormType.Batch, act_cls=nn.ReLU, pool=nn.AvgPool1d, pool_first=True, heads=4, mhsa=False, input_size=None, **kwargs):
        super().__init__()
        ...
        if(expansion ==1):
            ...
            layers = [
                ConvLayer(ni,  nh1, 1, **k0), # First 1x1 Conv (with activation)
...
class ResBlock_MCD_2(nn.Module):
    "Resnet block from `ni` to `nh` with `stride`"
    def __init__(self, expansion, ni, nf, stride=1, kernel_size=3, groups=1, nh1=None, nh2=None, dw=False, g2=1, dropout_prob = 0.05,
                 norm_type=NormType.Batch, act_cls=nn.ReLU, pool=nn.AvgPool1d, pool_first=True, heads=4, mhsa=False, input_size=None, **kwargs):
        super().__init__()
        ...
        if(expansion ==1):
            ...
            if dropout_prob > 0: # <-- Patch for first dropout layer
                layers.append(nn.Dropout(dropout_prob))
                
            layers.append(ConvLayer(nh2,  nf, kernel_size, groups=g2, **k1))
            
            if dropout_prob > 0: # <-- Patch for second dropout layer
                layers.append(nn.Dropout(dropout_prob))

        else: # Bottleneck block (expansion > 1)
...
class ResBlock_MCD_3(nn.Module):
    "Resnet block from `ni` to `nh` with `stride` following dropout guidelines"
    def __init__(self, expansion, ni, nf, stride=1, kernel_size=3, groups=1,
                 nh1=None, nh2=None, dw=False, g2=1, dropout_prob=0.05,
                 norm_type=NormType.Batch, act_cls=nn.ReLU,
                 pool=nn.AvgPool1d, pool_first=True, heads=4, mhsa=False,
        ...
        # --- BASIC BLOCK (expansion = 1) ---
        if expansion == 1:
            # conv1 + BN + ReLU
            conv1 = ConvLayer(ni, nh2, kernel_size, stride=stride,
                              groups=ni if dw else groups, **k0)

            # conv2 + BN (no ReLU)
            conv2 = ConvLayer(nh2, nf, kernel_size, groups=g2, **k1)

            # guideline: dropout AFTER the last BN
            layers = [conv1, conv2, nn.Dropout(dropout_prob)]

        # --- BOTTLENECK BLOCK (expansion > 1) ---
        else:
            # conv1 (1x1)
            conv1 = ConvLayer(ni, nh1, 1, **k0)

            # conv2 (3x3) or MHSA
            if mhsa == False:
                conv2 = ConvLayer(nh1, nh2, kernel_size, stride=stride,
                                  groups=nh1 if dw else groups, **k0)
            else:
                assert nh1 == nh2
                conv2 = MHSA1d(nh1, length=int(input_size), heads=heads)
                if stride == 2:
                    conv2 = nn.Sequential(conv2, nn.AvgPool1d(2, 2))

            # conv3 (1x1) with BN
            conv3 = ConvLayer(nh2, nf, 1, groups=g2, **k1)

            # guideline: dropout AFTER second BN (= after conv2)
            layers = [conv1, conv2, nn.Dropout(dropout_prob), conv3]
        ...
        idpath = []
        if ni != nf:
            idpath.append(ConvLayer(ni, nf, 1, act_cls=None, **kwargs))
        if stride != 1:
            idpath.insert((1, 0)[pool_first], pool(2, ceil_mode=True))
        self.idpath = nn.Sequential(*idpath)
...
class ResBlock_MCD(nn.Module):
    "Resnet block with correct dropout placement and original channel rules"
    def __init__(self, expansion, ni, nf, stride=1, kernel_size=3, groups=1,
                 nh1=None, nh2=None, dw=False, g2=1, dropout_prob=0.05,
                 norm_type=NormType.Batch, act_cls=nn.ReLU,
                 pool=nn.AvgPool1d, pool_first=True, heads=4, mhsa=False,
        ...
        # ----------------------
        # IMPORTANT: keep ORIGINAL logic (your architecture depends on it!)
        nf, ni = nf * expansion, ni * expansion
        # ----------------------
        ...
        k0 = dict(norm_type=norm_type, act_cls=act_cls, **kwargs)
        k1 = dict(norm_type=(NormType.BatchZero if norm_type == NormType.Batch else norm_type),
                  act_cls=None, **kwargs)
        ...
        if expansion == 1:
            ...
            # DROP internal dropout → causes MAE explosion
    ...
    def forward(self, x):
        out = self.convs(x)
        out = self.dropout(out)     # << dropout location 
