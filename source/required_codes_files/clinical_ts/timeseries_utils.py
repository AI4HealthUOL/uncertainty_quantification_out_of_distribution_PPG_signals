__all__ = ['nn_upsample','resample_labels','butter_filter', 'butter_filter_frequency_response', 'apply_butter_filter', 'save_dataset', 'load_dataset',
           ...
           'ChannelFilter', 'Transform', 'StaticTransform', 'TupleTransform', 'SequenceToSampleLabelTransform', 'aggregate_predictions', 'TimeseriesDatasetCrops_DANN']
...
class TimeseriesDatasetCrops_DANN(torch.utils.data.Dataset):
    """timeseries dataset with partial crops."""
    ...
    def __init__(self, df, output_size, chunk_length, min_chunk_length, memmap_filename=None, memmap_label_filename=None, npy_data=None, random_crop=True, data_folder=None, copies=0, col_data="data", col_lbl="label", cols_static=None, stride=None, start_idx=0, annotation=False, transforms=None, sample_items_per_record=1, fs_annotation_over_fs_data=1, return_sample_idx=False):
        ...
        if "source" in df.columns:
            self.timeseries_df_domain = np.array(df["source"]).astype(np.int64)
        else:
            raise ValueError(
                "TimeseriesDatasetCrops_DANN requires a 'source' column in df for DANN."
            )
        ...
        for df_idx,(id,row) in enumerate(df.iterrows()):
            ...
            for _ in range(copies+1):
                for i_s,i_e in zip(idx_start,idx_end):
                    ...
                    # -----------------------------------------
                    # replicate domain label for each crop
                    # -----------------------------------------
                    self.domain_mapping.append(self.timeseries_df_domain[df_idx])
                    
        #convert to np.array to avoid mp issues with python lists
        self.df_idx_mapping = np.array(self.df_idx_mapping)
        self.start_idx_mapping = np.array(self.start_idx_mapping)
        self.end_idx_mapping = np.array(self.end_idx_mapping)
        self.domain_mapping = np.array(self.domain_mapping)
    ...
    def _getitem(self, idx,start_idx_rel):
        ...
        domain = self.domain_mapping[idx]
        ...
        if self.return_sample_idx:
            if self.static:
                return df_idx, sample[0], sample[1], sample[2], domain
            else:
                return df_idx, sample[0], sample[1], domain
        else:
            if self.static:
                return sample[0], sample[1], sample[2], domain
            else:
                return sample[0], sample[1], domain
