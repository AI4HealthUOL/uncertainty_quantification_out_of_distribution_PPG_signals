...
class Main_PPG_MCD(Main_PPG):
    def __init__(self, hparams):
        ...
        if(hparams.architecture=="xresnet1d50_MCD"):
            #self.model = xresnet1d50_MCD(input_channels=hparams.input_channels, num_classes=num_classes, act_head="relu", ps_head=0.1, dropout_prob = hparams.dr) # original
            self.model = xresnet1d50_MCD(input_channels=hparams.input_channels, num_classes=num_classes, act_head="relu", ps_head=0.0, dropout_prob = hparams.dr)
            #print(self.model)
        elif(hparams.architecture=="xresnet1d101"):
            self.model = xresnet1d101(input_channels=hparams.input_channels, num_classes=num_classes)
        elif(hparams.architecture=="inception1d"):
    ...
    def on_test_epoch_end(self):
        ...
        # Get the current run’s folder (e.g., lightning_logs/version_7)
        save_dir = self.logger.log_dir

        # --------------------------
        # Save latents (train or test)
        # --------------------------
        if self.hparams.eval_on_train:
            with open(os.path.join(save_dir, 'train_latents.pkl'), 'wb') as f:
                pickle.dump(self.test_latents, f)
            return   # No further evaluation when evaluating on train

        else:
            # Save test latents
            with open(os.path.join(save_dir, 'test_latents.pkl'), 'wb') as f:
                pickle.dump(self.test_latents, f)

            # --------------------------
            # Save predictions and targets
            # --------------------------
            with open(os.path.join(save_dir, 'test_preds_cpu.pkl'), 'wb') as f:
                pickle.dump(self.test_preds_cpu, f)

            with open(os.path.join(save_dir, 'test_targs_cpu.pkl'), 'wb') as f:
                pickle.dump(self.test_targs_cpu, f)

            # --------------------------
            # Save aleatoric uncertainties (if GNLL)
            # --------------------------
            if self.hparams.gnll:
                with open(os.path.join(save_dir, 'test_ale_sbp.pkl'), 'wb') as f:
                    pickle.dump(self.test_ale_sbp, f)
                with open(os.path.join(save_dir, 'test_ale_dbp.pkl'), 'wb') as f:
                    pickle.dump(self.test_ale_dbp, f)

            # --------------------------
            # Save epistemic uncertainties
            # --------------------------
            with open(os.path.join(save_dir, 'test_epi_sbp.pkl'), 'wb') as f:
                pickle.dump(self.test_epi_sbp, f)
            with open(os.path.join(save_dir, 'test_epi_dbp.pkl'), 'wb') as f:
                pickle.dump(self.test_epi_dbp, f)

            # --------------------------
            # Optional: evaluation per dataloader
            # --------------------------
            for i in range(len(self.test_preds)):
